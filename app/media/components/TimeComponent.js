...
const TimeComponent = () => {
  ...
  const formatTime = (date) => {
    ...
    return (
      <div className="flex items-center justify-center">
        <div className="text-primary text-xl sm:text-3xl lg:text-5xl font-light">
          {hours}
          <span className="text-muted-foreground">:</span>
          {minutes}
        </div>
        <div className="ml-2 flex flex-col items-start">
          <span className="text-xs sm:text-sm lg:text-lg text-primary">{seconds}</span>
  ...
  return (
    <Card className="w-full h-full bg-gradient-to-br from-secondary/80 to-secondary/30 shadow-lg rounded-xl overflow-hidden transition-all duration-300 ease-in-out hover:shadow-xl hover:scale-[1.02]">
      <CardContent className="p-3 h-full flex flex-col">
        <div className="flex justify-between items-center mb-2">
          <div className="flex items-center space-x-1">
            <Calendar className="w-3 h-3 text-primary" />
            <span className="text-xs font-medium text-primary">{formatDate(time)}</span>
          </div>
